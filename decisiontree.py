...
def decision_tree_new_hierarchy(x_train, x_test, y_train, y_test, feature_names,pos):
    ...
    yf_pred = clf_train.predict(x_test[2])
    yg_pred = clg_train.predict(x_test[1])
    for k in range(0, len(yf_pred)):
        if int(yf_pred[k]) == 0:
            yg_pred[k] = 6
        elif int(yf_pred[k]) == 1:
            yg_pred[k] = 1

    ys_pred = cls_train.predict(x_test[0])
    for k in range(0, len(yg_pred)):
        if int(yg_pred[k]) == 6:
            ys_pred[k] = 8
        elif int(yg_pred[k]) == 1:
            ys_pred[k] = 2
        elif int(yg_pred[k]) == 4:
            ys_pred[k] = 6
        elif int(yg_pred[k]) == 7:
            ys_pred[k] = 9
        elif int(yg_pred[k]) == 5:
            ys_pred[k] = 7
        elif int(yg_pred[k]) == 2:
            ys_pred[k] = 3
        elif int(yg_pred[k]) == 6:
            ys_pred[k] = 8
        elif int(yg_pred[k]) == 1:
            ys_pred[k] = 2
